...
class InstitutionalStrategyService:
    def __init__(self):
        # Strict confluence thresholds based on the new conditions
        self.MIN_BULL_CONFLUENCE = 19
        self.MIN_BEAR_CONFLUENCE = 18
    ...
    def run_strategy(self, df_5m: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            df_4h['prev_EMA_9'] = df_4h['EMA_9'].shift(1)
            ...
            align(df_4h, ['STOCHk_14_3_3', 'STOCHd_14_3_3', 'close', 'EMA_9', 'prev_close', 'prev_EMA_9', 'EMA_50'], '4h')
            align(df_1d, ['4day_max_close', 'prev_low', 'prev_high'], '1d')

            zero_series = pd.Series(0, index=df_5m.index)
            hl_range = df_5m['high'] - df_5m['low']
            body_size = (df_5m['close'] - df_5m['open']).abs()

            # 5. Bullish Conditions (19 Rules)
            ...
            b6 = df_5m.get('4h_close', df_5m['close']) > df_5m.get('4h_EMA_9', df_5m['close'])
            b7 = df_5m.get('4h_prev_close', df_5m['close']) > df_5m.get('4h_prev_EMA_9', df_5m['close'])
            b8 = df_5m.get('4h_close', df_5m['close']) > df_5m.get('4h_EMA_50', df_5m['close'])
            ...
            s6 = df_5m.get('4h_close', df_5m['close']) < df_5m.get('4h_EMA_9', df_5m['close'])
            s7 = df_5m.get('4h_prev_close', df_5m['close']) < df_5m.get('4h_prev_EMA_9', df_5m['close'])
            s8 = df_5m.get('4h_close', df_5m['close']) < df_5m.get('4h_EMA_50', df_5m['close'])
            ...
            s16 = body_size > (0.55 * hl_range)
            s17 = df_5m.get('10_candle_range', zero_series) < (5 * df_5m.get('ATRr_14', hl_range))
            s18 = df_5m['volume'] > df_5m.get('avg_vol_20', zero_series)

            bearish_conditions = [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16, s17, s18]
            ...
            df_5m['is_bullish'] = df_5m['bullish_score'] >= self.MIN_BULL_CONFLUENCE
            df_5m['is_bearish'] = df_5m['bearish_score'] >= self.MIN_BEAR_CONFLUENCE
