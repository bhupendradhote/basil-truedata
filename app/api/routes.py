...
from app.services.strategy_service import InstitutionalStrategyService # NEW IMPORT

router = APIRouter()

td_service = TrueDataService()  
history_service = HistoricalDataService()
strategy_service = InstitutionalStrategyService() # NEW INSTANCE
...
# --- NEW STRATEGY ENDPOINT ---
@router.get("/strategy/{symbol}")
async def run_strategy(symbol: str, duration: str = Query("1 Y")):
    """
    Fetches 5-minute data, runs the multi-timeframe strategy, and returns chart data + signals.
    Note: We recommend pulling at least 1 Year (1 Y) of data so the 1-Week 50 EMA can calculate.
    """
    # Fetch raw 5m data
    ...
    # Extract signals for the frontend markers
    signals = []
